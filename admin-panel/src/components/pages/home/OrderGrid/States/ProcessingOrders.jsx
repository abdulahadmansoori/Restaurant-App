...
import { ClockCircleOutlined, CloseOutlined, CheckOutlined, SettingOutlined, UserOutlined, DoubleLeftOutlined, DoubleRightOutlined, EnvironmentOutlined, PhoneOutlined, MessageOutlined } from '@ant-design/icons';
...
const ProcessingOrders = () => {
    ...
    return (<>
        ...
        <div style={{ width: "80%", margin: "auto" }}>
            <Title level={5} style={{ margin: "2px 3px" }}>Orders In Processing</Title>
            <Collapse accordion>
                {orders.map((order, id) =>
                (<Panel header={order.orderName} key={order._id} extra={genExtra((order.updatedAt).split('T'),order.status, order.total)}>
                    ...
                    <Row>
                        ...
                        <Col className="gutter-row" span={8}>
                            <Button onClick={() => btnHandler(order._id, 'Pending')} style={{ margin: "0 2px", width: "48%" }}><DoubleLeftOutlined /> Pending</Button>
                            <Button type='primary' onClick={() => btnHandler(order._id, 'Delivered')} style={{ margin: "0 2px", width: "48%" }}>Deliver <DoubleRightOutlined /></Button>
                        </Col>
                    </Row>
                </Panel>)
                )}
...
const genExtra = (date, status, total) => {
    ...
    return (
        <>
            ...
            <Button style={{color:'orange', borderColor:'orange'}}>{icon} {status}</Button>
        </>)
};
