...
import { ClockCircleOutlined, CloseOutlined, CheckOutlined, SettingOutlined, UserOutlined, DoubleLeftOutlined, EnvironmentOutlined, PhoneOutlined, MessageOutlined } from '@ant-design/icons';
...
const DeliveredOrders = () => {
    ...
    return (<>
        ...
        <div style={{ width: "80%", margin: "auto" }}>
            <Title level={5} style={{ margin: "2px 3px" }}>Orders Delivered</Title>
            <Collapse accordion>
                {orders.map((order, id) =>
                (<Panel header={order.orderName} key={order._id} extra={genExtra((order.updatedAt).split('T'),order.status, order.total)}>
                    ...
                    <Row>
                        ...
                        <Col className="gutter-row" span={8}>
                            <Button type='primary' onClick={() => btnHandler(order._id, 'Processing')} style={{ margin: "0 2px", width: "48%" }}><DoubleLeftOutlined /> Processing</Button>
                        </Col>
                    </Row>
                </Panel>)
                )}
...
const genExtra = (date, status, total) => {
    ...
    return (
        <>
            ...
            <Button style={{color:'green', borderColor:'green'}}>{icon} {status}</Button>
        </>)
};
